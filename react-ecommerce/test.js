...
// const c=brand.map(c=>({value:c,label:c.charAt(0).toUpperCase()+c.slice(1).split('-').join(' ')}))
 const b=brand.map((b)=>({value:b,label:b,checked:false}))
console.log(b);

// (product.category).charAt(0).toUpperCase()+(product.category).slice(1)).split('-').join(' '))
